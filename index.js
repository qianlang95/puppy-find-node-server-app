...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            if (match) {
                // Assuming 'userID' is the field in your user document
                const token = jwt.sign({ userID: user.userID, email: user.email, role: user.role }, "jwt-secret-key", { expiresIn: '1d' });
                res.cookie('token', token);
                // Include 'userID' in the JSON response if you need it on the client-side
                res.json({ Status: "Success", role: user.role, userID: user.userID });
            } else {
                res.json({ Status: "Error", message: "The password is incorrect" });
            }
        } else {
            res.json({ Status: "Error", message: "No user record existed" });
        }
    } catch (err) {
